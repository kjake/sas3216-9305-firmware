...
eventually reverse-engineered exactly what makes these clones tick. This writeup
covers the byte-level differences, the checksum scheme, and why the two obvious
"fixes" both fail.
...
firmware, which is genuinely SAS3216 silicon, and you get a different failure:
...
I had two images to compare. One was a known-good backup from a working clone
(P15, IT mode), captured and shared by [Nialpo](https://forums.truenas.com/u/nialpo)
on the [TrueNAS forums](https://forums.truenas.com/t/help-finding-updated-firmware-for-avago-sas3216-9305-16i-hba-card/62254).
The other was the stock Broadcom 9305-16i P15 image. Same size to the byte (959,848),
with identical headers down to the firmware version field. The difference is tiny:
...
there, so you get `port enable: FAILED` after a long hang.
...
last so the image sums back to zero. One detail cost me a rebuild; the balancer has
to be located by that `03000000…74340000` signature, not a fixed offset. P16 grew
the NVDATA header by 0x70 bytes, and a hardcoded offset put my checksum write inside
the build-date string.

## P16.12 does the hard part
...
## One byte I couldn't place (until I could)
...
**Oracle.** Apply the full transform to stock 16i P15, recompute the checksums, and
compare against the known-good clone backup. It has to come out byte-for-byte
identical:
...
**Hardware.** The P16.12 output flashed clean on a SAS3216 clone (HP Z220,
...
- These clones are SAS3216 silicon on a 16i board. They need SAS3216 identity plus
  the 16i internal PHY map; no stock firmware ships that combination.
